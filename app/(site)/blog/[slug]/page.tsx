...
const ptComponents = {
  block: {
    h2: ({ children, value }: any) => { // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  return (
    <>
      <style>{`
        ...
        .ava-sm { width:44px; height:44px; border-radius:50%; background:linear-gradient(135deg,var(--mint),var(--mint-soft)); display:grid; place-items:center; font-family:var(--font-display); color:var(--teal); font-size:18px; flex-shrink:0; }
        .a-meta .b .name { font-weight:500; font-size:14px; }
        .a-meta .b .role { color:var(--muted); font-size:13px; margin-top:2px; }
        .a-meta .b .sub { color:var(--muted); font-family:var(--font-mono); font-size:11px; letter-spacing:.12em; text-transform:uppercase; margin-top:4px; }
        ...
        .a-cover::after { content:""; position:absolute; inset:0; background:repeating-linear-gradient(to right,transparent 0,transparent 19px,rgba(255,255,255,.05) 19px,rgba(255,255,255,.05) 20px),repeating-linear-gradient(to bottom,transparent 0,transparent 19px,rgba(255,255,255,.05) 19px,rgba(255,255,255,.05) 20px); pointer-events:none; }
        .a-cover.has-image::after { display:none; }
        .a-cover .clabel { position:absolute; left:40px; bottom:32px; color:rgba(255,255,255,.9); font-family:var(--font-display); font-size:96px; line-height:.9; z-index:1; }
        .a-cover .ctag { position:absolute; right:40px; top:32px; color:rgba(255,255,255,.7); font-family:var(--font-mono); font-size:11px; letter-spacing:.14em; text-transform:uppercase; z-index:1; }
        ...
        .related a { background:var(--surface); border:1px solid var(--line); border-radius:16px; padding:24px; display:flex; flex-direction:column; gap:12px; text-decoration:none; color:inherit; }
      ...
      <section className="a-hero">
        <div className="wrap">
          ...
          <div className="a-meta">
            <div className="ava-sm">{initials(post.author?.name)}</div>
            <div className="b">
              <div className="name">{post.author?.name}</div>
              {post.author?.role && <div className="role">{post.author.role}</div>}
              <div className="sub">{formatDate(post.publishedAt)}{post.readTime ? ` · ${post.readTime} min read` : ""}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="wrap">
        <div className={`a-cover${post.coverImage ? " has-image" : ""}`}>
          {post.coverImage && (
          ...
          return (
            <div className="body-layout">
              <div className="toc">
                ...
                )) : <span style={{ color: "var(--muted)", fontSize: 11 }}>Add H2 headings to generate contents.</span>}
              </div>
              ...
              <div className="content">
                {post.body?.length ? (
                  ...
                  <p style={{ color: "var(--muted)", fontStyle: "italic" }}>No body content yet — add text in Sanity Studio.</p>
                )}
              </div>
